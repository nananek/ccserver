...
export function buildControlMcpServer(deps) {
  ...
  server.tool(
    'list_group_sessions',
    'List all sessions in this orchestration group (workers and orchestrator) with role, app, cwd, live status, autoYes (whether automatic permission-approval is enabled; null when the member has no live session, e.g. a restored one) and idleForMs (ms since each session last produced output; null when no live session exists). A member expected to be working but showing a large idleForMs may be stuck -- confirm with read_output before assuming progress.',
    {},
