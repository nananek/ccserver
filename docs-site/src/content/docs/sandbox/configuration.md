...
- **IPv4 アドレス (`192.168.0.5`) と、ドットのない名前 (`gitea`、`localhost`) は、そのまま書けます**。内部のホストも許可できますが、書いた時点で「このサーバーが接続してよい相手」になります。URL 側は文字列の完全一致なので、`127.0.0.1` を書いても `127.1` や `2130706433` のような別の表記は一致しません。
