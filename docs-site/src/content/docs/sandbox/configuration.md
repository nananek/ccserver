...
- **`browseRoots` を設定していない場合 (既定)**: この起動時チェックは**何もしません**。browseRoots が無ければ任意のディレクトリをセッションの cwd にできるため、「サンドボックスから書き込めない場所」を機械的に判定する方法がないからです。とくに `--file` を省略した既定の保存先 (`sandbox.config.json` と同じディレクトリ = 通常は ccserver のチェックアウト内) は保護されません。**ccserver のチェックアウト外で、セッションを開くことのない絶対パスを明示的に指定してください。**
