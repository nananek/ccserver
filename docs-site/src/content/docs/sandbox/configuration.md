...
**保存先はサンドボックスから書き換えられない場所にしてください。** `browseRoots` を設定している場合、その配下はセッションの cwd に指定でき、サンドボックス内のエージェントが集計ファイルを読み書きできます。集計ファイルを browseRoots 配下に置くと件数の改ざんや記録の抑制が可能になるため、有効化した集計ファイルが browseRoots 配下にある場合、ccserver は他の内部状態ファイルと同様に**起動を拒否します**。`--file` にはセッションから見えない絶対パス (ccserver の状態ディレクトリなど) を指定してください。`file` は絶対パスである必要があり (相対パスは無効として無視されます)、`--file` を省略した既定の保存先 (`sandbox.config.json` と同じディレクトリ) は同じ起動時チェックで守られます。
