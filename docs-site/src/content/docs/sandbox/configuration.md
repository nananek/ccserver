...
**保存先はサンドボックスから書き換えられない場所にしてください。** `browseRoots` を設定している場合、その配下はセッションの cwd に指定でき、サンドボックス内のエージェントが集計ファイルを読み書きできます。集計ファイルを browseRoots 配下やプロジェクトディレクトリ内に置くと、件数の改ざんや記録の抑制が可能になるため、`--file` にはセッションから見えない絶対パス (ccserver の状態ディレクトリなど) を指定してください。`--file` を省略した既定の保存先 (`sandbox.config.json` と同じディレクトリ) は、ccserver が「browseRoots に自身の状態ファイルが含まれる場合は起動を拒否する」ため安全です。
