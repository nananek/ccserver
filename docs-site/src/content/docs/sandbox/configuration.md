...
| `network` | `{ isolate: false, mode: "enforce", allowedHosts: [], deniedHosts: [] }` | ネットワーク分離 ([下記](#ネットワーク分離)参照)。`isolate` は起動時ポリシーで、Linux では分離の on/off (`true`=ブローカー+境界ありで enforce 起動)、macOS では初期 state の選択 (`true`=enforce 開始、`false`=open 開始。常時 armed のため `false` でも無制限にはならない) を意味します。`mode` は `"enforce"`/`"audit"`、`allowedHosts`/`deniedHosts` は完全一致か先頭ドット (`.example.com`) のみの許可/拒否リスト (各最大200件)。設定 UI (設定 → ネットワーク分離) からも編集可能で、`allowedHosts`/`deniedHosts` の保存は稼働中セッションへ自動反映されます。 |

## ネットワーク分離

外向き通信をホスト側の CONNECT プロキシ (`network-broker.js`) 経由に限定します。TLS はそのままパススルーするプレーンな HTTP CONNECT プロキシで、中身を復号しません。Linux と macOS で `isolate` の意味が異なります。

- **bwrap (Linux)**: `isolate: true` かつ rootlesskit/slirp4netns/newuidmap が揃っているホストでのみ、bwrap 起動が私有ネットワーク名前空間 + in-netns ファイアウォール (iptables 優先、nft フォールバック) でラップされ、ブローカーのポートと DNS 以外への通信を構造的に遮断します (初期 state=enforce)。起動後は、そのセッションのトグルでいつでも enforce (許可リストのみ) / open (一時的に全通信許可) を切り替えられます — ただし切り替えられるのは境界がある間だけで、`isolate: false` (既定) で起動したセッションはブローカー自体が起動せず従来通り無制限で、切り替え自体が現れません。rootlesskit 系ツールが1つでも欠けているホストでは、`isolate: true` を指定していても境界を作らず、従来どおりの非隔離 bwrap 起動にフォールバックします (起動は失敗せず、警告ログのみ)。
- **seatbelt (macOS)**: 常時 armed です。`isolate` の値に関わらず、すべての起動でブローカーが起動し、プロファイルは `(allow network*)` の代わりにブローカーのループバックポートのみを許可するルールになります (ツール依存はありません)。無制限の起動は存在しません。`isolate` は初期 state の選択に過ぎず、`true`=enforce 開始・`false` (既定)=open 開始で、稼働中の 🌐 トグルで再起動なしにいつでも enforce/open を切り替えられます。ただし open でも直結 TCP/UDP は不可で proxy 経由のみとなるため、proxy を無視するツールは open でも直結できません。bwrap のようなカーネルレベルの境界ではなく、同一UIDのプロセスは `KERN_PROCARGS2` 経由でブローカーのトークンを読み取れてしまうため、あくまで defense-in-depth です。
