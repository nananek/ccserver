...
- PRF の salt はパスキーごとのランダム値です。**アンロックごとの salt ローテーションは行いません** (導入当初は「アンロックのたびに新しい salt へ差し替え、一度漏れた PRF 出力を次のアンロックで失効させる」設計でしたが、次 salt の PRF 出力はクライアント申告値でサーバーが検証できないため、アンロック儀式を 1 回完遂できるクライアントが任意の値で鍵ラップを作り替え、本来のオーナーを恒久ロックアウトできる問題 (vuln_scan M4) への対応として廃止しました)。このため、**一度漏れた PRF 出力は自動的には失効しません**。漏洩が疑われる場合は、Vault の削除・再作成と GitHub 側の鍵登録の更新を検討してください。
