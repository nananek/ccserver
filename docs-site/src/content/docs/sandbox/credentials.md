...
- サンドボックス起動オプションの「GPG Vaultで署名・SSH pushする」(`gpgVault`) を有効にした状態でVaultが未作成、または修正前バージョンで作成された無効化済みボルトだと、**起動自体が明確なエラーで拒否されます** (黙って機能なしで起動することはありません)。一方、Vaultが**ロック中** (作成済みだが未アンロック) の場合は、起動自体は拒否されません — サーバーログに警告が出た上でそのまま起動し、アンロックされるまで署名/SSH pushはエラーで失敗します (稼働中にVaultがロックされた場合と同じ挙動で、アンロック後は同一セッション内で自動的に使えるようになります)。
