...
- レビュー発注時に `git rev-parse "origin/<branch>"` で**対象 SHA を確定**し、レビュアーにはブランチ名ではなく**その SHA を detached でチェックアウト**させます (`git checkout --detach <sha>`)。SHA は 16 進 40 文字なのでペイロードを運べず、レビュー中にブランチが動いても対象がずれません。
...
- **ブランチ名はシェルに渡る前に検証します。** git の ref 名は `;` `|` `&` `$` バッククォート `(` `)` を許容し、ブランチ名を決めるのは被レビュー側です。`git check-ref-format "refs/heads/<branch>"` で確認したうえでこれらを含む名前は拒否し、どの用途でも必ずクォートしてください。SHA でチェックアウトする運用はこの面も同時に塞ぎます。
...
- **技術的な強制ではありません。** サーバー側にこれを強制する仕組みはありません。`gh pr create` は findings ドキュメントの有無を見ませんし、この段の文言を固定しているテストも**文言が削除・弱化されたこと**しか検出できません (矛盾する例外条項を追記されても通りますし、悪意ある変更はテストごと書き換えられます)。これはオーケストレーターが従うことで成り立つ規範であり、だからこそ手順は「システムの保証」ではなく「あなたへの指示」として書かれています。
