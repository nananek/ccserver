...
コンボ起動のワークフローでは、**PR を作成する前に OpenCode ワーカーによる攻撃者視点レビューを必ず 1 回通します**。オプションの追加チェックではなく、通さないとマージに進まない必須ゲートです。オーケストレーター注入テンプレート (`server/ws/orchestrator-template.md`) に段として組み込まれており、流れは次のようになります。

**実装 → 実装ワーカーのセルフレビュー (最大 3 ラウンド) → 攻撃者視点レビュー → 最終レビュー → push → PR**
...
- **`app: 'opencode'` を明示的に指定**します。実装側と別のモデル系統を当てることで、実装ワーカーの盲点をそのまま引き継がないようにするのがこのゲートの狙いです。
- **対象ブランチは detached でチェックアウト**させます: `git fetch origin && git checkout --detach origin/<branch>`。実装ワーカーの worktree が同名ブランチを既にチェックアウトしており、git は同一ブランチを 2 つの worktree で同時にチェックアウトすることを拒否するためです。事前に実装ワーカーへ `git push -u origin <branch>` させておく必要があります (リモートに無いブランチはレビューできません)。
- **発注時に「実際に攻撃を再現すること」を明示**します。これを書かないとレビューは静的な読み合わせに退化します。悪意ある入力を実際に組み立て、パス・URL・ペイロードを作って動かし、何が起きるかを観測させてください。
- **再現できたものと机上の推測を明確に区別させます。** 各 finding が「再現した exploit (手順と観測された出力つき)」なのか「未検証の仮説」なのかを必ず書かせます。両者が混ざったレビューは対応の優先順位を付けられません。
- findings は `publish_doc` (例: キー `"attack-review"`) で公開させ、実装ワーカーにはキーだけを渡して対応させます。**対応後にもう一度攻撃者レビューを回すかはオーケストレーターの判断**です (修正の規模と、部分的にしか塞がっていない finding の有無で決めます)。

このゲートについて、2 つの誤解を明示的に否定しています。
