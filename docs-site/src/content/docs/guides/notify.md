...
- **タイトルはサーバーが組み立てます** (`<アプリ名> · <プロジェクト名>`)。エージェントが指定したタイトルは本文側へ畳み込まれるので、**別のセッションやシステム自身からの通知を騙ることはできません**。
  ...
  ただしこれは**通知ブリッジ経由の通知に限った話**です。エージェントが `notify` MCP ツールを自分で呼んだ場合、タイトルはエージェントが指定したものになります (それがツールの用途なので)。どちらの経路でも、**発信元を示す `_from:` フッター (PWA 通知では `attribution` フィールド) は常にサーバーが付けるもの**なので、「誰からの通知か」はそちらで判断してください。本文中に `_from:` と書いても無害化されるため、フッターを騙ることはどちらの経路でもできません。
