...
- 本文中の `_from:` は無害化されます。無描画の文字 (ゼロ幅スペース、結合文字、異体字セレクタ、点字の空白 U+2800、私用領域など) を途中に挟んで回避しようとしても、それらごと無害化します。

  ただし**見た目が似ているだけの文字は潰しません**。全角の `＿from：`、キリル文字の `о` を使った `_frом:`、語中に通常の空白を入れた `_fr om:` などは残ります (最後のものは空白として描画されるので、そもそも見た目が違います)。同形異字の網羅は原理的に不可能なので、保証は「**ccserver 自身のフッターとバイト単位で同一の見た目のものは作れない**」までです。
