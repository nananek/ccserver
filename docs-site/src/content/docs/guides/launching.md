...
- **クリップボード同期 (OSC 52)**: opencode がターミナルに書き込む OSC 52 シーケンスをブラウザが解釈します (xterm.js は OSC 52 を無視するため、ccserver 側で処理)。システムクリップボードへの書き込みは、確認ダイアログで「許可」したときだけ反映されます (許可は 1 回の書き込みごとです。「拒否 (以後確認しない)」を選ぶと、そのターミナルセッションの間は以後の書き込みも確認なしで無視します)。
