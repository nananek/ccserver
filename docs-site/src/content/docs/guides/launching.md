...
- **緑は時間だけで決めていません。** 各 CLI が稼働中だけフッターに出す中断キーのヒント (Claude Code は `esc to interrupt`、opencode は `esc interrupt`) が**無いこと**と、画面が静止していることの**両方**を満たしたときだけ緑になります。スピナーが回っている間は緑になりません。
