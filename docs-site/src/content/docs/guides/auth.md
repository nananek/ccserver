...
セッションがあるだけでは登録できません (セキュリティ監査 F2 対応: セッション Cookie を盗んだ攻撃者が自分のパスキーを登録し、GPGボルトの乗っ取りに繋げる経路を塞ぐため)。登録には次のどちらかが必要です。

- **ステップアップ**: 登録済みのパスキーでのユーザー検証付き認証を 5 分以内に行っていること。パスキーでログインした直後のセッションはこれを満たします。それ以外の場合は、登録ボタンを押すと既存パスキーでの本人確認が自動的に求められます。
