...
# パスキーを登録する必要がある場合 (初回セットアップ / 全パスキー紛失時のリカバリ):
npm run login-token -- --allow-passkey-registration
```
...
`--allow-passkey-registration` を付けて発行したトークンでログインしたセッションだけが、**パスキーを1つだけ**登録できます (登録に成功した時点でその権限は消費されます)。付けずに発行したトークンはログインのみ可能です。**パスキーが1つも無い初回セットアップでもこのフラグが必要です** (暗黙の例外はありません)。

### フロー2: パスキー登録

ログイン済み状態で `Settings` タブの「パスキー」セクションから新しいパスキーを登録できます。登録できるパスキー数に上限はなく、複数デバイスでの複数登録に対応します (削除 UI は現時点では未実装です)。

セッションがあるだけでは登録できません (セキュリティ監査 F2 対応: セッション Cookie を盗んだ攻撃者が自分のパスキーを登録し、GPG Vaultの乗っ取りに繋げる経路を塞ぐため)。登録には次のどちらかが必要です。

- **ステップアップ**: 登録済みのパスキーでのユーザー検証付き認証 (ステップアップ) を 5 分以内に行っていること。パスキーでのログインはステップアップとして扱いません。登録ボタンを押すと、既存パスキーでの本人確認が自動的に求められます。
- **登録権限付きトークン**: フロー1 で `--allow-passkey-registration` を付けて発行したトークンでログインしたセッション (1回限り)。

新しいパスキーが登録されると、サーバーログと通知チャネル (Discord/Webhook 等) に通知されます。
...
すべてのパスキーが使えなくなった場合は、フロー1 (ワンタイムトークン) で再ログインしてください。新しいパスキーを登録するには、トークンを `--allow-passkey-registration` 付きで発行します。認証自体を無効化したい場合は `CCSERVER_AUTH_MODE=none` に変更して再起動します (DB 上での動的な無効化は行いません)。
