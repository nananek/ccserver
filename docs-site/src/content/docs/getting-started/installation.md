...
```bash
npm run setup           # ドライラン: 何がどこに作られるか表示 (何も変更しない)
npm run setup -- --yes  # 実行
```

設定・データ・状態を XDG のディレクトリに用意します。

| | 場所 |
|---|---|
| 設定 | `~/.config/ccserver/` |
| データ (DB・federation 鍵・グループファイル) | `~/.local/share/ccserver/` |
| 状態 (セッション・グループ・予約プロンプト) | `~/.local/state/ccserver/` |

**新規インストールでも実行が必要です。** 実行するまで Web UI は新しいセッションやグループの作成を拒否します (実行中セッションの閲覧と再アタッチは可能です)。既存インストールをアップグレードする場合は、リポジトリ配下にあった設定・状態ファイルがここへ移行されます — **サーバーを停止してから**実行してください。

詳細と全オプションは [設定モデル](/ccserver/reference/configuration-model/) を参照してください。
